...
def dataset_sampler(dataset, n_support, n_query, tgt_id, inductive=False, sample=False):
    ...
    if not inductive:
        ...
        if sample:
            sample_num = min(len(query_list), n_support)
            query_list = sample_from_candi(query_list, sample_num)

    return dataset[support_list], dataset[query_list], support_list, query_list
